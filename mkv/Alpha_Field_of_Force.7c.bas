...
Sub Initialization()
  Screen 20,24,3
  Windowtitle " The Alpha Preferred Speed and the Fields of Force. - Jan. 30, 2010."
...
' c_speed = 1 / ((1 / beta) - alpha)                                  '.577 pixel per loop in order to mach the scan speed (1 / c) minus alpha speed.
  c_speed = .71                                                       'regular speed, theoretically .707 or sin(45, .71 for Doppler-contracted waves.
  x_alpha = 407
  x_twin_A = x_width
  x_origin = -30
  x_scanner = -1100
...
'*********************************************************************
  ...
  Locate 47, 88: ? "January 30, 2010. This program may be"
...
Sub Display()
  ...
  amplitude = 346 / Sqr(distance)
  For x = x_alpha To x_alpha + field_contraction * 3 * lambda         'node and antinode envelope.
  ...
  Line(x-1, y_graph + previous)-(x, y_graph), black
  Line(x_alpha - 20, y_graph - 48)-(x_alpha + field_contraction * 3 * lambda + 20, y_graph + 48), black, B
  Line(x_alpha - 19, y_graph - 47)-(x_alpha + field_contraction * 3 * lambda + 19, y_graph + 47), black, B
...
'*********************************************************************
  ...
  For x = 0 To x_width: For y = 0 To y_height
    If wave_display Then
      If y = y_center Then
        potential = .075 * (trend(x-1,y) + trend(x+1,y) + trend(x,y-1) + trend(x,y+1))
...
Sub Wave_Emitter()
  ...
  For x = x_alpha + field_contraction * 3 * lambda + 20 To x_width   'white envelope according to distance.
    ...
    If distance < .2 * lambda Then
      ...
      y_coord = 75 * Cos(radian)
    Else y_coord = 2.85 * lambda / Sqr(distance)
    End If
    Line(x, y_graph - y_coord)-(x, y_graph + y_coord), white
...
'     gaussian = pi ^ -((distance / lambda) ^ 2)                      'the normal distribution works for a stationary emitter only.
      If distance < lambda / 4 Then
        ...
        If y = 0 Then
          y_coord = y_graph - 46 * potential * Cos(radian)
...
' Applying the Lorentz transformations for twin B.********************
  ...
  For x = 0 To x_alpha - 20                                           'white envelope according to distance.
    ...
    If distance < .2 * lambda * g_Lorentz Then
      radian = 1.9 * pi * distance / lambda                           'adjusting a sinusoidal curve to the 3 / 4 lambda central antinode.
      y_coord = 88 * Cos(radian)
    Else y_coord = 2.85 * lambda / Sqr(distance)
    End If
    Line(x, y_graph - y_coord)-(x, y_graph + y_coord), white
    Line(x-1, y_graph - previous - 1)-(x, y_graph - y_coord - 1), black
...
' a maximum as shown above (see Sub Wave_Emitter).
  ...
  Select Case Pulse_B                                                 'code 1: waiting ZERO amplitude to set the threshold.
    Case 1: If amplitude < .01 Then                                   'set .1 instead of .01 for shorter wavelength.
              pulse_B = 2
              threshold = -.5 * lambda + 2                            'waiting for the 1/4 or 3/4 phase to start emitting.
            End If
            Exit Sub
  ...
  For x = x_first To x_last                                           'the impulse zone will contract.
    ...
    For y = -.25 * lambda To .25 * lambda
      ...
      If distance < .25 * lambda Then
        ...
        If y = 0 Then
          y_coord = y_graph - 65 * potential * curve
