...
skipped_frames = 3: scanner = 0: bitmap = 0                           'set bitmap = 1 for BMP image sequence.
Initialization()
...
Sub Initialization()
  Screen 21,24,3
  Windowtitle " The Time Scanner and the Twin Paradox - Jan. 7, 2010"
...
' Display.
  ...
  Locate 63,122: ? "January 9, 2010. This program may be"
...
Sub Wave_Emitter_Doppler()
  ...
  amplitude = Abs(Cos(t_prime))

  If pulse_B = 1 Then
    If amplitude > .99 Then pulse_B = 2 Else Exit Sub
...
'   t' = g * t - b * x
'   However, the time shift is 6 times exaggerated.
'   The goal is to balance the forward vs. backward amplitude.
