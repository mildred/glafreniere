...
Dim Shared As Integer matrix_page, work_page, visible_page, luminance_1, luminance_2, click, wheel, bitmap, offset
...
Dim Shared As Single wave_speed, ratio, brightness, move_frame, t_time, t_prime, g_Lorentz, k_Dewavrin
...
' JOCELYN MARCOTTE'S 2-D WAVE ALGORITHM (CREATED IN 2006).            "THE PAST IS A GUIDE TO THE FUTURE"
    ...
    For x = x_start To x_stop: For y = y_start To y_stop
      ...
      trend(x,y) = .25 * orthogonal + present(x,y) - past(x,y)        'trend extrapolation with .5 slower waves.
    Next: Next
    ...
    move_frame += .5 * beta                                           'waves are 50% slower.
...
Sub Flat_Mirrors()
  ...
  If iteration < 1.4 * parabola_radius / g_Lorentz Then               'temporary corner reflector.
...
' The vertical mirror needs a slight offset to balance shorter Doppler contracted waves. Shorter waves travel a
' bit slower as a result of a quantum effect. The normal offset is 3 because there are 5 unequal reflection steps.
  For y = -path_width To path_width                                   'vertical mirror.
    trend(x_vertical_mirror+offset-4, y_beam_splitter + y) = .946 * trend(x_vertical_mirror+offset-4, y_beam_splitter + y)'.8^(1/4)
    trend(x_vertical_mirror+offset-3, y_beam_splitter + y) = .88  * trend(x_vertical_mirror+offset-3, y_beam_splitter + y)'.6^(1/4)
    trend(x_vertical_mirror+offset-2, y_beam_splitter + y) = .795 * trend(x_vertical_mirror+offset-2, y_beam_splitter + y)'.4^(1/4
    trend(x_vertical_mirror+offset-1, y_beam_splitter + y) = .669 * trend(x_vertical_mirror+offset-1, y_beam_splitter + y)'.2^(1/4)
    trend(x_vertical_mirror+offset, y_beam_splitter + y) = 0
  Next

  If frame = 0 Then                                                   'drawing vertical mirror.
...
Sub Initialization()
  Windowtitle " Motion Optics 02 - Orthogonal parabola and wave beam."
  display = "amplitude"
  iteration = 0
...
'  If x_width < y_height Then x_width = y_height
  ...
  y_center = .5 * y_height
  path_width = 5 * lambda
  parabola_radius = y_height
  y_parabola = y_height - 20
  x_emitter = x_center
  ...
  Select Case beta                                                    'adjusting transparency and vertical mirror offset.
    Case 0
      If scale = 1 Then
        transparency = .929
        offset = 3                                                    'middle point; there are 5 unequal mirror steps.                                
      Else
        transparency = .975
      End If
    Case Is Sin(pi/4)
...
'  transparency = 1 - (.071 / scale)                                   'by trial and error.
'  transparency = .929' 1 - .071  beta = 0
'  transparency = .975' scale = 2
'  transparency = .950' scale = 1, beta = .7071
...
' Display.
  ...
  Locate 46,99: ? "October 17, 2009. This"
...
'  Line(x_emitter / scale, 0)-(x_emitter / scale, 2 * lambda / scale), white
  ...
  For x = -path_width To path_width Step scale
    ...
    Pset((x + x_emitter) / scale, (y_parabola - y + 1) / scale), green
  Next
  Line((path_width + x_emitter) / scale, (y_parabola - y) / scale)-((path_width + x_emitter) / scale + 1, (y_parabola - y) / scale + 23), green,  b'drawing the reflector.
  Line((-path_width + x_emitter) / scale, (y_parabola - y) / scale)-((-path_width + x_emitter) / scale - 1, (y_parabola - y) / scale + 23), green,  b
  For x = -g_Lorentz * path_width To g_Lorentz * path_width Step scale
    ...
    Pset((x + x_emitter) / scale, (y_parabola - y) / scale + 1), white
  Next
  Line((g_Lorentz * path_width + x_emitter) / scale, (y_parabola - y) / scale)-((g_Lorentz * path_width + x_emitter) / scale + 1, (y_parabola - y) / scale + 23), white,  b
  Line((-g_Lorentz * path_width + x_emitter) / scale, (y_parabola - y) / scale)-((-g_Lorentz * path_width + x_emitter) / scale - 1, (y_parabola - y) / scale + 23), white,  b

  Line(x_emitter / scale, (y_parabola - 4 * lambda) / scale)-(x_emitter / scale, y_parabola / scale), white
  For y = lambda To 4 * lambda Step lambda
...
'  Locate 48, 13: If g_Lorentz = 1 Then ? "  Regular Parabola "; Else ? " Squashed Parabola ";
  Locate 64, 19: ? "glafreniere.com";
  Color black, background: Locate 20, 100: Print stop_pulsating; iteration, pulses * 2 * lambda,
  If iteration > pulses * 2 * lambda Then Print "stop"
End Sub
...
Sub Circular_Generator_Doppler()
  ...
  amplitude = 2000 / lambda ^ 2
  target = pulses * 2 * lambda
  
'  If stop_pulsating = 0 Then
'    If iteration > 2 * (pulses - 1) * lambda / g_Lorentz Then         'wave speed is .5 pixel per cycle.
...
'  End If
  ...
  Elseif iteration > target Then                                      'stop pulsating gradually.
    amplitude = amplitude * (4 * lambda - (iteration - target)) / (4 * lambda)
    If iteration > target + 4 * lambda Then stop_pulsating = 2: Exit Sub
  End If
'  If stop_pulsating = 1 Then                                          'stop pulsating gradually.
'    amplitude = amplitude * (target - iteration) / (4 * lambda)       'useful for beam splitter, may be omitted.
'  End If
