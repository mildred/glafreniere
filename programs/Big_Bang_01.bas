...
Sub Display()
  ...
  For radian = 0 To 2 * pi Step pi / 24                               'reflecting spots.
    ...
    beta = x_coord / t_Lorentz                                        'speed of the current spot before acceleration.
    ...
    Circle(x_A + x_coord, y_center + y_coord), 4, dark,,,1
    Paint (x_A + x_coord, y_center + y_coord), dark, dark
  Next
  ...
  For radian = 0 To 2 * pi Step pi / 24                               'reflecting spots.
    ...
    For x = -8 To 8
      ...
      For y = -10 To 10
        ...
        If distance < 10 Then trend(x_A - move_frame + x + x_coord, y_center + y + y_coord) = soft * trend(x_A - move_frame + x + x_coord, y_center + y + y_coord)
      Next
    Next
  Next
End Sub
...
'  iteration = 600
  ...
  lambda = 24
  brightness = 64
  soft = .99                                                          'very faint reflection, yet on a large area.
  x_scale = -240
