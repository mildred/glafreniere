...
Sub Flat_Mirrors()
  ...
  x = iteration * (wave_speed / beta - wave_speed * beta)             'phase wave (Lorentz's t' = 0) position at the center.
  x -= (y_parabola - y_emitter + y_parabola + .5 * pulses * lambda) / g_Lorentz / wave_speed - .5 * pulses * lambda / Tan(Asin(beta))'adjusting position according to the delay.
  x -= 23                                                             'fine tuning.

  If x > -off_axis And x <= 0 Then
    For x_pixel = x To x + off_axis
      If x_pixel > -damping_zone Then trend(x_pixel, 0) = 0
  ...
  Elseif x > 0 And x <= x_width Then
    For x_pixel = x - off_axis To x + off_axis
      If x_pixel > -damping_zone And x_pixel < x_width + damping_zone Then trend(x_pixel, 0) = 0
      If frame = 0 And x_pixel Mod scale = 0 Then Pset(x_pixel, 0), white
    Next
  Elseif x > x_width And x < x_width + off_axis Then
    For x_pixel = x - off_axis To x
      If x_pixel < x_width + damping_zone Then trend(x_pixel, 0) = 0
      If frame = 0 And x_pixel Mod scale = 0 Then Pset(x_pixel, 0), white
    Next
  End If
...
Sub Initialization()
  Windowtitle " The Bradley Aberration of Light - Plain"
  display = "amplitude"
  iteration = 0
  brightness = .7071
  x_center = .5 * x_width
...
'  If off_axis > damping_zone Then off_axis = damping_zone
  ...
  parabola_radius = 2 * y_height - pulses * lambda / 2
...
' Display.
  ...
  Locate 46,99: ? "February 24, 2009. This"
...
' The phase wave speed is given by 1 / beta. A correction is added because the true wave speed is .707 pixel per loop.
' In addition, it is slowed down because the frame of reference is also moving forward according to: wave_speed * beta.
  x = iteration * (wave_speed / beta - wave_speed * beta)             'phase wave (Lorentz's t' = 0) position at the center.
  x -= (y_parabola - y_emitter + y_parabola + .5 * pulses * lambda) / g_Lorentz / wave_speed - .5 * pulses * lambda / Tan(Asin(beta))'adjusting position according to the delay.
  x -= 22                                                             'fine tuning.
  Line(x,0)-(x, 3 * lambda), white                                    'phase wave position for upper plane mirror.
  ...
  Locate 43, 1:       ? " Aberration of Light"
  Locate 44, 1:       ? " Aberration of Light"
  Locate 45, 1:       ? " Aberration of Light";
  ...
  Locate 45, 105: If g_Lorentz = 1 Then ? " Regular Parabola"; Else ? "Squashed Parabola";
