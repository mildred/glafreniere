...
Dim Shared As Single amplitude, phase, distance, radian, potential, wave_speed
...
Sub Initialization()
  ...
  x_center = .5 * x_width
  y_center = .5 * y_height
  lambda = 64                                                         'lambda in pixels, multiple of 4 advisable and
  half_lambda = .5 * lambda                                           '                  compatible with c = 1 pixel.
  wave_speed = .707                                                   'Sin 45°.
...
Sub Keyboard_Management()
  ...
  Case "P": Sleep: If in_key = Chr(27) Then Sleep: in_key = ""
  End Select
...
Sub Wave_Display()
  ...
  For x = 0 To x_width: For y = 0 To y_height
    ...
    End If
       Pset(x,y), Rgb(r,g,b)
  Next: Next
  For y = 0 to 4 * lambda Step lambda
    Line(x_emitter - 5, y_center - y)-(x_emitter + 5, y_center - y), gray
  Next
End Sub
...
Sub Wave_Generator()
  If iteration >= 4 * lambda + .2 * lambda Then Return                'stop pulsating.
  amplitude = 10000 / lambda ^ 2.5
  phase = iteration * wave_speed * 2 * pi / lambda
