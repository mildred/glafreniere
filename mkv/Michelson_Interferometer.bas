...
x_width  = 768 * scale
y_height = 768 * scale
skipped_frames = 4 * scale / g_Lorentz - 1
Initialization()
...
' JOCELYN MARCOTTE'S 2-D WAVE ALGORITHM (CREATED IN 2006).            "THE PAST IS A GUIDE TO THE FUTURE"
  ...
  If bitmap > 0 Then                                                  'initialize bitmap = 1 for bitmap sequence.
    ...
    Bsave file, 0
    If bitmap = 429 Then Bsave ".5 429 scale 2 trans .938 .7071.bmp", 0': End
    bitmap += 1
...
Sub Flat_Mirrors()
  ...
  If iteration < scale * (650 + 540 * beta ^ 4) Then                  'temporary corner reflector.
...
Sub Initialization()
  ...
  x_vertical_mirror = x_emitter + g_Lorentz * y_beam_splitter         'contraction according to Lorentz.
...
' Mr. Anselme Dewavrin pointed out that Euler's method yields incorrect results when applied to sinusoidal cycles.
' On the one hand, the normal step for covering a full wavelength step by step is 2 * pi / lambda.
' On the other hand, considering the quantum effect, the accurate step is rather given by: 2 * Sin(pi / lambda).
...
' waves traveling forward are severely contracted and this produces a speed desiquilibrium. This problem may be
' solved by using a longer wavelength, but the time difference may also be cancelled by a small wave speed correction.
' Please note that the number of steps per wavelength is given by lambda / Sin(pi/4) because of the 4-neighbor algorithm.
  ...
  wave_speed = Sin(pi/4) - .367 * k_Dewavrin

' BEAM SPLITTER TRANSPARENCY - The formula below is based on results by trial and error.
' Transparency depends on the mirror angle, but it is unclear why the wavelength is also involved.
' beta vs transparency for scale = 1 and lambda = 40:
'   0: .866; .5: .88; .7071: .905
' beta vs transparency for scale = 2, hence lambda = 80:
'   .0: .93; .5: .938; .7071: .95
' transparency = (1 - .14 / scale) + (.12 * beta ^ 3 / scale)

  If scale = 1 then                                                   'transparency ratio for the beam splitter.
    transparency = (1 - 5.36 / lambda) + (4.41 * beta ^ 3 / lambda)
  Else
    transparency = (1 - 5.6 / lambda)  + (4.52 * beta ^ 3 / lambda)
  End If
...
' Display.
  ...
  Locate 2,100: ? Using "Dewavrin's constant .####"; k_Dewavrin
'  Locate 3,100: ? Using "Wave Speed          .###"; Sin(pi/4) * (1 - k_Dewavrin * Sin(pi/4))
  Locate 3,100: ? Using "Wave Speed          .####"; wave_speed
  Locate 4,100: ? Using "Transparency        .####"; transparency
  ...
  Locate 46,99: ? "February 25, 2009. This"
...
'  Next
  ...
  Line((x_emitter - path_width) / scale, 0)-((x_emitter - path_width) / scale, y_height / scale), cyan
  Line((x_emitter + path_width) / scale, 0)-((x_emitter + path_width) / scale, y_height / scale), cyan
  Line((x_emitter - path_width) / scale, 0)-((x_emitter + path_width) / scale, 0), cyan
  Line(0, (y_beam_splitter - path_width) / scale)-(x_width / scale, (y_beam_splitter - path_width) / scale), cyan
  Line(0, (y_beam_splitter + path_width) / scale)-(x_width / scale, (y_beam_splitter + path_width) / scale), cyan
  Line(x_emitter / scale + y_beam_splitter / scale-1, (y_beam_splitter - path_width) / scale)-(x_emitter / scale + y_beam_splitter / scale-1, (y_beam_splitter + path_width) / scale), cyan
