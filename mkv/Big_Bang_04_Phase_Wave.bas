  ...
  For x = -radius To radius                                           'printing the galaxies.
    For y = -radius To radius                                         'scanning x and y, one pixel at a time.
      If x_y(x, y) Then                                               'copying the matrix color only when x_y(x, y) code = 1.
        beta_x = 0                                                    'central galaxy.
        beta_y = 0
        Print_Galaxy_Single()

        beta_x = beta_D                                               'critical oblique fastest galaxy.
        beta_y = beta_F
        Print_Galaxy_Single()
        beta_x = -beta_D
        beta_y = beta_F
        ...
        beta_x = Sqr(beta_F^2 +(g_Lorentz_F*beta_D)^2)                'axial fastest galaxy, same absolute speed for comparison.
        beta_y = 0
        Print_Galaxy_Single()
        ...
        Print_Galaxy_Single()
        beta_x = 0
        beta_y = Sqr(beta_F^2 +(g_Lorentz_F*beta_D)^2)
        Print_Galaxy_Single()
        beta_x = 0
        beta_y = -Sqr(beta_F^2 +(g_Lorentz_F*beta_D)^2)
        Print_Galaxy_Single()
...
'  Paint( x_A, y_center), 12, 12
  ...
  Line(03, 169)-(309, 247), white, B                                  'equation box.
  Line(04, 170)-(308, 246), dark, BF
  Locate 12, 02: ? "t[x]'  = g[x] * t     - ";Chr(225);"[x] * x "
  Locate 13, 02: ? "t[xy]' = g[y] * t[x]' - ";Chr(225);"[y] * y"
  Locate 14, 02: ? "x'     = g[x] * x     + ";Chr(225);"[x] * t"
  Locate 15, 02: ? "y'     = g[y] * y     + ";Chr(225);"[y] * t[x]'"
  
  Locate 17, 02: ? Chr(225);"[xy] = Sqr(";Chr(225);"[x]^2 + (g[x] * ";Chr(225);"[y])^2)"
  Locate 18, 02: ? "g[xy] = Sqr(1 - ";Chr(225);"[xy]^2)"
...
'  Locate 47, 02: ? "lead to a null result."
  ...
  Locate 06, 125: ? "The phase wave is the consequence"
  Locate 07, 125: ? "of the t[xy]' equation shown on the"
  Locate 08, 125: ? "left. It is required in order to"
  Locate 09, 125: ? "obtain forward Doppler-contracted"
  Locate 10, 125: ? "waves whose amplitude is higher."

  Locate 45,148: ? "June 4, 2010"
...
' follows correctly the phase wave. Such a behavior allows the wave
...
' expansion at the same rate as the Universe does.
  ...
  Else
    radian = 2 * pi * distance / (radius - 2) / 4 
    If Cos(t_radian) > 0 Then
      g = 255 * Cos(t_radian) * Cos(radian)
      b = 128 * Cos(t_radian) * Cos(radian)
      r = 0
    Else
      r = 255 * -Cos(t_radian) * Cos(radian)
      b = 128 * -Cos(t_radian) * Cos(radian)
      g = 0
    End If
  ...
  End If
  If x = 0 And y = 0 Then
    If beta_x > .93                        Then Locate 51, 112: Print Using "Lower right galaxy phase  #.########"; Cos(t_radian)
    If beta_x = beta_D And beta_y = beta_F Then Locate 52, 112: Print Using "Reference galaxy (right)  #.########"; Cos(t_radian)
  End If
  If x = radius Then
