...
? " Created February 2009 by Gabriel LaFreniere.":?:?
...
x_screen = 1280 * scale
y_screen = 1024 * scale
Dim Shared As Integer x_start, y_start, x_stop, y_stop, x_center, y_center, x_mouse, y_mouse, x_pixel, y_pixel
Dim Shared As Integer x_coord, y_coord, y_previous, x_squared, x_emitter, y_emitter, x_straight_emitter
...
lambda = 32 * scale
beta = .5                                                             'Sin(pi/4) = .7071; Sin(pi/3) = .866
...
x_width  = 1280 * scale
y_height = 720 * scale
skipped_frames = (4 + 1) * scale / g_Lorentz
Initialization()
...
'    Flat_Mirrors()
  ...
  If bitmap > 0 Then                                                  'initialize bitmap = 1 for bitmap sequence.
    ...
    Color red, background: Locate 56, 105: Print file
    Bsave file, 0
    bitmap += 1
...
Sub Initialization()
'  Windowtitle " The Bradley Aberration of Light - Plain - Stationary"
  Windowtitle " The Phase Wave"
  display = "amplitude"
  iteration = 0
  brightness = 1
  x_center = .5 * x_width
  ...
  path_width = g_Lorentz * x_center
  path_width = x_center - 12
  If x_width < y_height Then x_width = y_height
  y_parabola = y_height - 1
  y_emitter = y_center
  If beta < .5 Then
    y_parabola = y_height - 100
    y_emitter = y_parabola / 2
  End If
  parabola_radius = 2 * (y_parabola - y_emitter)
  x_emitter = x_center
  y_beam_splitter = y_center
  mirror_angle = Atn(g_Lorentz / 1)
  target = pulses * lambda / g_Lorentz / wave_speed
'  x_start = -damping_zone + 2
'  y_start = -damping_zone
  x_start = 0                                                         'damping zone useless here.
  y_start = 0
...
' Display.
  ...
  Locate 46,99: ? "February 15, 2009. This"
...
Sub Parabolic_Reflector()
  ...
  For x = -path_width To 0                                            'PARABOLA LEFT SIDE.
    y = (x / g_Lorentz) ^ 2 / 2 / parabola_radius
    trend(x + x_emitter, y) = 0                                       'upper parabolic reflector.
    trend(x + x_emitter, y_parabola - y) = 0                          'lower parabolic reflector.
    If y_previous - y > 1 Then                                        'filling empty spaces.
      trend(x + x_emitter, y+1) = 0
      Pset ((x + x_emitter-1)/scale, (y+1)/scale), white
      trend(x + x_emitter, y_parabola - y-1) = 0
      Pset ((x + x_emitter-1)/scale, (y_parabola - y-1)/scale), white
    End If                                                            'this side doesn't need dithering.
    y_previous = y
  Next

  For x = 0 To path_width                                             'PARABOLA RIGHT SIDE.
    y = (x / g_Lorentz) ^ 2 / 2 / parabola_radius
    trend(x + x_emitter, y) = 0                                       'upper parabolic reflector.
    trend(x + x_emitter, y_parabola - y) = 0                          'lower parabolic reflector.

    If y - y_previous > 1 Then                                        'filling empty spaces.
      trend(x + x_emitter-1, y-1) = 0                                 'upper parabola empty spaces.
      Pset ((x + x_emitter-1)/scale, (y-1)/scale), white
      trend(x + x_emitter-1, y_parabola - y+1) = 0                    'lower parabola empty spaces.
      Pset ((x + x_emitter-1)/scale, (y_parabola - y+1)/scale), white
 
' This side needs dithering because of the medium displacement.
' The move_frame variable below indicates the exact medium position with respect to the current pixel. 
' The correct tempering value is given by the square root.
' Dithering must be applied to the first pixel on the left of each horizontal row only.

      trend(x + x_emitter-2, y-1) = Sqr(move_frame) * trend(x + x_emitter-2, y-1)'upper parabolic reflector.
'      Pset (x + x_emitter-2, y-1), red
      trend(x + x_emitter-1, y) = Sqr(move_frame) * trend(x + x_emitter-1, y)
'      Pset (x + x_emitter-1, y), blue
      trend(x + x_emitter-2, y_parabola - y+1) = Sqr(move_frame) * trend(x + x_emitter-2, y+1)'lower parabolic reflector.
'      Pset (x + x_emitter-2, y_parabola - y+1), red
      trend(x + x_emitter-1, y_parabola - y) = Sqr(move_frame) * trend(x + x_emitter-1, y)
'      Pset (x + x_emitter-1, y_parabola - y), blue
    Elseif y - y_previous = 1 Then
      trend(x + x_emitter-1, y) = Sqr(move_frame) * trend(x + x_emitter-1, y)'upper parabolic reflector.
'      Pset (x + x_emitter-1, y), green
      trend(x + x_emitter-1, y_parabola - y) = Sqr(move_frame) * trend(x + x_emitter-1, y_parabola - y)'lower parabolic reflector.
'      Pset (x + x_emitter-1, y_parabola - y), green
    End If
    y_previous = y
  Next
...
Sub Wave_Display()
  ...
  Color white, black
  Locate 2, 1: ? " Green curves: confocal regular parabolae."
  Locate 3, 1: ? " White curves: squashed parabolae."
  Locate 5, 1: ? " The squashed parabola is still a"
  Locate 6, 1: ? " parabola and the focal plane"
  Locate 7, 1: ? " distance remains identical"
  Locate 8, 1: ? " to that of the regular"
  Locate 9, 1: ? " parabola."
  
  Locate 2, 115: ? " The phase wave speed is given by: 1 / beta."
  Locate 3, 117: ? " On this plane, Lorentz's t' time does not"
  Locate 4, 122: ? " change. Thus, it allows one to scan"
  Locate 5, 124: ? " the whole system in such a way that"
  Locate 6, 128: ? " the Doppler effect is canceled."
  Locate 7, 132: ? " This is why my Time Scanner"
  Locate 8, 137: ? " easily reproduces the"
  Locate 9, 140: ? " well known Lorentz"
  Locate 10,143: ? " Transformations."
'  Locate 38, 1:       ? " The Bradley"
'  Locate 39, 1:       ? " Aberration of Light"
  Locate 39, 1:       ? " The Phase Wave"
...
'  Locate 43, 43:  ? "       The regular parabola transforms curved waves into plane waves."
'  Locate 44, 43:  ? "  After reflection on the plane mirror, it rather transforms plane waves into"
'  Locate 45, 43:  ? "curved converging waves which finally produce the well-known Airy disk pattern.";
  Locate 40, 143: ? "Special thanks to"
  Locate 41, 139: ? "Philippe Delmotte and"
  Locate 42, 130: ? "Jocelyn Marcotte, the creators"
  Locate 43, 125: ? "of this amazing virtual wave medium."
  Locate 45, 125: ? "Gabriel LaFreniere  glafreniere.com";
  Locate 47, 138: ? " Iteration"; iteration; " ";
  Locate 38, 155: Print Using "#####"; Int(iteration / (skipped_frames + 1));
  
' The phase wave speed is given by 1 / beta in wavelengths per period (or in seconds per light second) on condition that c = 1.
' A correction is added because the wave speed is actually .707 pixel per loop.
' In addition, it is slowed down even more because the frame of reference is also moving forward according to: wave_speed * beta.
  If beta = 0 Then
    x = -1
  Else
    x = iteration * (wave_speed / beta - wave_speed * beta) / scale   'phase wave position (Lorentz's t' = 0 or unchanged).
  End If
  x -= 945                                                            'fine tuning for transverse standing waves (beta = .5).
  If x > 1239 Then x -= 1239                                          'fine tuning for circular standing waves (beta = .5).
  Do While x > 1239
    x -= 1000                                                         'constant scanning.
  Loop
  Line(x,0)-(x, y_height / scale), white                              'phase wave position.
End Sub
