...
do'    MODÉLISATION DE L'ÉTHER EN DEUX DIMENSIONS AVEC EFFET DOPPLER, le 27 déc. 2005.
  ...
  if afficher = 0 or clic = 1 then                        'afficher une fois sur deux.
    ...
    if len(saisie$) then
      ...
      select case saisie$
        ...
        case "M+":run "Ether10.exe"                       'flèche droite.  
    ...
    if clic = 1 then
      select case ligne
        ...
        case 21 : capture = 0: gosub Inverser
      ...
      case 37: locate, 39: print ligne37$;
               if xSouris < 400 then
                 ...
                 if clic = 1 then run "Ether08.exe"
                  ...
                  gosub flecheDroite
                 if clic = 1 then  run "Ether10.exe"
                end if      
    end select
    color , fond
...
'    locate 30, 80: print using "#.## sec"; timer - temps;
    temps = timer
    capture = capture + 1: if capture > 999 then gosub Doppler
'    print ". Image"; capture
  end if
  afficher = afficher + 1
...
for x = -1 to cote
  ...
  ligne = .5 + ySouris / 16
  if clic = 1 and ligne = 36 then end
...
locate 35
locate , 70: print "Le 18 avril 2007. Ce programme"
