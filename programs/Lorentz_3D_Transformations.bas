...
'  Next:Next
  ...
  Locate 03, 03: ? "The Lorentz All Azimuth Transformations"
...
' y_prime    = g_Lorentz_y * y         + beta_y * t_prime_x
  ...
  Locate 38, 03: ? "visible here. Amazingly, it perfectly"
  Locate 39, 03: ? "matches the Doppler effect pattern."
...
Sub Initialization()
  Screen 21,24,3
  Windowtitle " The Lorentz all Azimuth Transformations"
...
Sub Phase_Wave()
  ...
  For x = -100 To 100                                                 'scanning Lorentz's image.
    ...
    For y = -100 To 100
      ...
      distance = Sqr(x_squared + y^2)
      If distance < 98 Then
        t_radian = t_prime_xy * 2 * pi / lambda + .7 * pi             'wave period in radians - .6 pi for synchronization purpose.
        phase = Cos(t_radian)
        If phase > 0 Then
          r = 0
          g = 1.5 * 255 * phase
