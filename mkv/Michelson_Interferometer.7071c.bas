...
scale = 2'1                                                           'set scale = 2 for more accurate results.
lambda = 40 * scale
beta = Sin(pi/3)
...
'    if iteration = 1260 then sleep:end
  ...
  If bitmap > 0 Then                                                  'initialize bitmap = 1 for bitmap sequence.
    ...
    Bsave file, 0
'    If bitmap = 424 Then Bsave "1.424.866_12.bmp", 0                    '424 and 721 for beta = .707 scale = 1
'    If bitmap = 721 Then Bsave "1.721.866_12.bmp", 0: End               '300 and 510 for beta = 0    scale = 1
    bitmap += 1                                                       '346 and 589 for beta = .5   scale = 1
...
' bit slower as a result of a quantum effect. The normal offset is 3 because there are 5 unequal reflection steps.
  For y = -path_width To path_width                                   'vertical mirror.
    trend(x_vertical_mirror-1, y_beam_splitter + y) = .946 * trend(x_vertical_mirror-1, y_beam_splitter + y)'.8^(1/4)
    trend(x_vertical_mirror+0, y_beam_splitter + y) = .88  * trend(x_vertical_mirror+0, y_beam_splitter + y)'.6^(1/4)
    trend(x_vertical_mirror+1, y_beam_splitter + y) = .795 * trend(x_vertical_mirror+1, y_beam_splitter + y)'.4^(1/4
    trend(x_vertical_mirror+2, y_beam_splitter + y) = .669 * trend(x_vertical_mirror+2, y_beam_splitter + y)'.2^(1/4)
    trend(x_vertical_mirror+3, y_beam_splitter + y) = 0
  Next
...
Sub Initialization()
  ...
  If scale = 1 Then x_vertical_mirror = x_vertical_mirror - 12 * beta^4'small correction for slower shorter waves.
...
'  Select Case beta                                                   'adjusting transparency and offset.
'    Case 0
'      offset = 3                                                     'vertical mirror offset (see above: - 8 * beta^3).
'      If scale = 1 Then
'        transparency = .929
'      Else
'        transparency = .975
...
'  transparency = .975' scale = 2, beta = 0 
'  transparency = .950' scale = 1, beta = .7071

  If scale = 1 Then
       transparency = .929 + .08 * beta ^ 3.85                        'by trial and error starting from .929 (beta = 0).
  Else transparency = .975 + .1  * beta ^ 8
  End If
...
' Display.
  ...
  Locate 46,99: ? "January 18, 2009. This"
