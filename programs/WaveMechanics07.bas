...
Dim Shared As Single x_prime, y_prime, x_squared, y_squared, g_Lorentz, radius_Lorentz
Dim Shared As Single beta_B, beta_C, beta_Scale, theta, amplitude, phase,  scanner, printer, center, variable, c
...
bitmap = 0                                                            'set bitmap = 1 to create a bitmap image sequence.
work_page = 1
Screen 21,24,3
Initialization()
    ...
    Select Case key
      ...
      Case "R"                                                        'reversing the process for experiments.
        If image > lambda Then
          image -= lambda
          scanner += lambda * c / beta_C
          printer += lambda * g_Lorentz * c / beta_C
          red_frame -= lambda * c * beta_Scale
          green_frame += lambda * c * beta_B
          Screenset matrix_page
          Line(0, 0)-(1279, 719), background, bf
          Screenset work_page
        End If
    End Select
    Do: Loop While Len(Inkey)                                         'clearing buffer.
  End If
...
Sub Graphics():
  ...
  If scanner < -1 Then
    Sleep(10)                                                         'allowing more time for CPU's external tasks.
    Exit Sub
  Else
...
' DISPLAYING THE TRANSFORMED WAVE EMITTER (moving backward along with B, hence beta_B) USING THE LORENTZ TRANSFORMATIONS
      x_prime = g_Lorentz * x_coord + beta_B * t_time                 'Lorentz's x' coordinate in wavelength units.
      t_prime = g_Lorentz * t_time  - beta_B * x_coord                'Lorentz's t' time (actually the emitter phase) in 2 pi units.
' **********************************************************************************************************************
...
' DISPLAYING THE TRANSFORMED WAVE EMITTER MOVING FORWARD ALONG WITH C USING THE LORENTZ TRANSFORMATIONS ****************
      x_prime = g_Lorentz * x_coord + beta_C * t_time                 'Lorentz's x' coordinate in wavelength units.
      t_prime = g_Lorentz * t_time  - beta_C * x_coord                'Lorentz's t' time (actually the emitter phase) in 2 pi units.
' **********************************************************************************************************************
...
'   B is moving backward (beta_B = -.5 c) but its center is located at x = 0 before undergoing the Lorentz transformations.
    x_coord = 0
    x_prime = g_Lorentz * x_coord + beta_B * t_time                   'according to Lorentz reversed transformations,
    ...
    If scanner > x_point - 67 Then
      t_prime = g_Lorentz * t_time - beta_B * x_coord                 'Lorentz's reversed time equation (moving backward: beta_B = -.5 c).
...
'   B' is moving backward along with B, APPARENTLY 10 wavelengths apart.
    ...
    If scanner > x_point - 67 Then
      t_prime = g_Lorentz * t_time - beta_B * x_coord                 'Lorentz's reversed time equation (backward: beta_B = -.5 c).
      Locate 2, 1: Print Using " t =  ##.#  sec."; t_prime
      Line(39, 14)-(86, 32), black, b
      Line(40, 15)-(85, 31), black, b
      Get(8, 14)-(128, 32), t_prime_BC
...
'   C is moving forward (beta_C = .5 c) but its center is located at x = 0 before undergoing the Lorentz transformations.
    x_coord = 0
    x_prime = g_Lorentz * x_coord + beta_C * t_time                   'according to Lorentz reversed transformations.
    x_point = zero_point + x_prime * lambda
    If x_point - radius_Lorentz <= scanner Then
      If x_point < scanner - radius_Lorentz - 1 Then
        Get(x_center - radius_Lorentz, y4_center - radius)-(x_center + radius_Lorentz, y4_center + radius), contracted_body'***************************
    ...
    If scanner > x_point - 67 Then
      t_prime = g_Lorentz * t_time - beta_C * x_coord                 'Lorentz's reversed time equation (forward: beta_C = .5 C).
      Locate 2, 1: Print Using " t'=  ##.#  sec."; t_prime
      Line(39, 14)-(86, 32), black, b
      Line(40, 15)-(85, 31), black, b
      Get(8, 14)-(128, 32), t_prime_BC
...
'   C' is moving along with C, APPARENTLY 10 wavelengths apart.
    x_coord -= 10
    x_prime = g_Lorentz * x_coord + beta_C * t_time
...
'***********************************************************************************************************************
    ...
    red_frame += c * beta_Scale                                       'wavelength red scale, will not contract.
    ...
    If scanner > red_frame - 67 Then                                  'displaying the red scale "absolute" time.
    ...
    green_frame -= c * beta_B                                         'wavelength green scale, will contract to .866.
    ...
    scanner -= c / beta_C                                             'scanning leftward.
    printer -= g_Lorentz * c / beta_C
...
'scanner = -1
    If scanner < 0 Then
      ...
      Line(0, y4_center - radius)-(x_center + radius_Lorentz, y4_center + radius), background, bf'removing material bodies.
      ...
      Locate 33,2: Print "                                                  "'erasing previous text.
...
Sub Initialization()
  ...
  beta_C = .5                                                         '50% of the speed of light forward (rightward).
  beta_B = -beta_C                                                    'same speed as C, yet backward.
  theta = Asin(beta_C)                                                'transverse wave angle.
  g_Lorentz = Cos(beta_C)                                             'Lorentz contraction factor for C, same factor for B.
...
'                                                                      beta_Scale = beta_C / 2 = .25 would be incorrect.
  beta_Scale = -.267949192
  ...
  image = 0
  images = lambda / beta_C                                            'number of images per period.
  images = 50
...
'  printer = 910
  ...
  Locate 53,107:Print "C and C':   (beta+beta) / (1+beta*beta)  ="; (beta_C + beta_C) / (1 + beta_C * beta_C); " c."
  Locate 54,107:Print "Contraction for transformed C and C':  g ="; Sqr(1- ((beta_C + beta_C) / (1 + beta_C * beta_C))^2)
...
'  Line(450, 841)-(836, 1024), gray,  b
'  Line(451, 842)-(836, 1024), black, b
'  Line(836, 842)-(836, 1024), white
'  Line(837, 841)-(837, 1024), white
...
'  Locate 64, 2: ? "Gabriel LaFreniere  glafreniere.com";
  Locate 63,121:? "August 20, 2009. This program may be"
