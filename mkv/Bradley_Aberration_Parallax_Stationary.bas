...
Dim Shared As Integer x_coord, y_coord, x_squared, x_straight_emitter, x1_small_parabola, x2_small_parabola
...
lambda = 20 * scale
beta = 0                                                              'Sin(pi/4) = .7071; Sin(pi/3) = .866
...
x_width  = 1280 * scale
y_height = 724 * scale
skipped_frames = 4 * scale / g_Lorentz - 1                            'displays one image out of x using scale 1 at rest.
Initialization()
...
' JOCELYN MARCOTTE'S 2-D WAVE ALGORITHM (CREATED IN 2006).            "THE PAST IS A GUIDE TO THE FUTURE"
    ...
    For x = x_start To x_stop: For y = y_start To y_stop
      ...
      trend(x,y) = .25 * orthogonal + present(x,y) - past(x,y)        'this trend extrapolation calculus
    Next: Next                                                        'produces slower waves: wave_speed = .5
...
Sub Initialization()
  Windowtitle " The Bradley Aberration of Light - Parallax - Stationary System"
  display = "amplitude"
  iteration = 0
  brightness = 1
  x_center = .5 * x_width
  ...
  parabola_radius = 2 * y_height - 2 * lambda
  y_parabola = y_height
  y_small_parabola = .92 * y_height
  x1_small_parabola = .3 * x_width
  x2_small_parabola = .7 * x_width
...
' Display.
  ...
  Locate 46,99: ? "February 16, 2009. This"
  Locate 47,99: ? "program may be freely copied,"
  Locate 48,99: ? "distributed, or modified.";
  Locate 58,10: ? "Full Screen: Press Alt + Enter."
  Locate 60,10: ? "The screen resolution should be at least 1280x1024 and match the same aspect ratio (5:4)."
  Color green_text
...
Sub Parabolic_Reflectors()
  ...
  For x = -.3 * path_width To .3 * path_width
  ...
  If frame = 0 Then                                                   'drawing the reflector.
    For x = -.3 * path_width / g_Lorentz To .3 * path_width / g_Lorentz Step scale
      y = x ^ 2 / 2 / small_parabola_radius                           'regular parabolae.
      Pset((x + x1_small_parabola) / scale, (y_small_parabola - y) / scale), green
      Pset((x + x2_small_parabola) / scale, (y_small_parabola - y) / scale), green
    Next
    For x = -.3 * path_width To .3 * path_width Step scale
...
Sub Wave_Display()
  ...
  Line(x1_small_parabola / scale, (y_small_parabola - 200) / scale)-(x1_small_parabola / scale, y_small_parabola / scale), gray
  Line(x2_small_parabola / scale, (y_small_parabola - 200) / scale)-(x2_small_parabola / scale, y_small_parabola / scale), gray
  ...
  Line((2*x1_small_parabola-x_emitter) / scale, y_emitter)-(x1_small_parabola / scale, y_small_parabola / scale), gray
  Line((2*x2_small_parabola-x_emitter) / scale, y_emitter)-(x2_small_parabola / scale, y_small_parabola / scale), gray
...
'  x = iteration * (wave_speed / beta - wave_speed * beta)             'phase wave (Lorentz's t' = 0) position.
'  x -= 2 * y_height / g_Lorentz / wave_speed - x_width                'adjusting position according to the delay.
'  x += 59                                                             'fine tuning.

  x = iteration * (wave_speed / beta - wave_speed * beta)             'phase wave (Lorentz's t' = 0) position.
  x -= 1500                                                           'adjusting position according to the delay.

'  Line(x,0)-(x, 357), white                                           'phase wave position for upper plane mirror.
'  Line(x-20,337)-(x+20, 337), white
'  Circle(x, 337), 10, white
'  Line(x_emitter / scale, 0)-(x_emitter / scale, y_height), gray
  
'  For y = .5 * lambda To 3 * lambda Step lambda / 2                   'wavelength scale.
'    Line(x - 10, y)-(x + 10, y), white
...
'  Next
  ...
  Locate 40, 1:       ? " The Bradley"
  Locate 41, 1:       ? " Aberration of Light"
...
'    Color white
  ...
  Locate 42, 69: ? "PARALLAX WITHOUT MOTION"
  Locate 44, 45: ? " The emitter finite distance and its off-axis position produce spherical"
  Locate 45, 46: ? " aberration, coma and astigmatism. However, parallax is still obvious.";
  Locate 41,143: ? "Special thanks to"
  Locate 42,139: ? "Philippe Delmotte and"
  Locate 43,130: ? "Jocelyn Marcotte, the creators"
  Locate 44,125: ? "of this amazing virtual wave medium."
  Locate 45,125: ? "Gabriel LaFreniere  glafreniere.com ";
  Locate 50,104: Print " Iteration"; iteration; " ";
End Sub
