...
Dim Shared As Integer frame, skipped_frames, line_number, axial, code, unit, scan(-4 To x_width + 4, y_height)
...
' Circle(x_coord,y_A),.5*(x_C-x_A)/g_Lorentz_X,white,,, 1/g_Lorentz_X 'expanding ellipsoid mirror.
  ...
  For radian = 0 To 2 * pi Step pi / sphere_radius                    'elliptic mirrors.
    If radian < .4 Then                                               'right (partial).
      code = 1
    Elseif 2 * pi - radian < .4 Then code = 1                         'right.
    Elseif Abs(radian - pi) < .4 Then code = 1                        'left.
    Elseif Abs(radian - 3 * pi / 2) < .4 Then code = 1                'top
    Elseif Abs(radian - pi / 2) < .4 Then code = 1                    'bottom.
    Else code = 0
    End If
'    If code Then
...
'      Circle(x_A - move_frame + x_coord, y_center + y_coord), 1, white,,,1
'      Paint (x_A - move_frame + x_coord, y_center + y_coord), white, white
'    End If
  Next
...
'        Pset(x_A + x_prime, y_center + y_prime), white
  ...
  x_coord = x_A - move_frame + x_prime                                'white galaxy C moving forward (beta_C = 0.8 c).
  ...
  Paint( x_B, y_center), bright, 2                                    'bright: will be erased by waves.
...
Sub Elliptic_Reflector()                                              'see display() above for calculus details.
  For radian = 0 To 2 * pi Step pi / sphere_radius                    'elliptic mirrors.
    If radian < .05 Then                                               'right (partial).
      code = 1
    Elseif 2 * pi - radian < .05 Then code = 1                         'right.
    Elseif Abs(radian - pi) < .05 Then code = 1                        'left.
    Elseif Abs(radian - 3 * pi / 2) < .05 Then code = 1                'top
    Elseif Abs(radian - pi / 2) < .05 Then code = 1                    'bottom.
    Else code = 0
    End If
    If code Then
      x_coord = beta_B * sphere_radius * Cos(radian)                  'axial coordinate before transformation (proportional to speed).
      ...
      For x = -10 To 10
        ...
        For y = -10 To 10
          ...
          If distance < 10 Then trend(x + x_coord, y + y_coord) = soft * trend(x + x_coord, y + y_coord)
        Next
      Next
    End If
  Next
...
Sub Emitter_B()                                                       'axial emitter B: beta = 0.5.
  ...
  If t_prime - t_start > 1.73 Then pulse_B = 0: Exit Sub              'adding about .73 phase offset in 2-D in order to cancel some residual energy.
...
Sub Initialization()
  Screen 21,24,3
  Windowtitle " The Big Bang - Four orthogonal expanding mirrors."
