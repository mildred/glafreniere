...
Declare Sub Curve_Display():           Const white =      RGB(255,255,255)
Declare Sub Display():                 Const yellow =     RGB(255,255,0)
Declare Sub Energy_Display():          Const background = RGB(225,225,225)
Declare Sub Black_and_White_Display()
...
Dim Shared As Integer bitmap, y_height = 504, x_width = 776, x_center, y_center
...
Dim Shared As String number, deleting, file_name, in_key, mode, mouse_line_58
...
Do
	...
	If Len(in_key) Then
		...
		Select Case in_key
			Case "A": If mode <> "curve"  Then mode = "curve":  Update()
	...
	Locate 63, 2:
	Print Using "Distance to Focal Plane ###.##"; Abs(x_mouse - x_center) / lambda;
	...
	Select Case mouse_line
		...
		Case 58: If mode = "curve"  Then Else Print mouse_line_58
	...
	If click = 1 Then	'---------------------------------------------------------- MOUSE MANAGEMENT.
		...
		Select Case mouse_line
			...
			Case 58: If mode <> "curve"  Then mode = "curve":  Update()
...
Sub Axial_Graphic()'********************************************** AXIAL GRAPHIC
	Line(0, y_height + 58)-(1279, y_height + 200), background, bf                 'erasing previous curves.
	Locate 45, 99: Print "glafreniere.com       Gabriel LaFreniere     October 10, 2010."
	...
	For x = 0 To x_width / 2 - 1
		...
		y_wave_2 = .114 * luminance(x, 0) * Sin(period(x, 0) + phase)              'symmetry.
   ...
   y_previous = .114 * luminance(x_coord, 0)
	...
	Line(x_width, y_height)-(x_width, 719), black
End Sub
...
Sub Concave_Source()'******************** STANDARD CALCULUS FOR A CONCAVE SOURCE
   Curve_Display()
   If mode = "curve" Then Exit Sub
	Line(0, 0)-(1279, y_height), gray, bf                                         'darker area.
	Color white, gray: Locate 28, 10: Print "Please wait..."
   Color black, background
	'the two simplified algorithms in Hemispheric_Source and Spherical_Source Subs were useful in order
	'to check the accuracy of the standard one below, but they are not used in this program:
...
Sub Cursor_Angle() '******************************* ADJUSTING THE APERTURE ANGLE
	...
	If previous_angle <> aperture_angle Then
		...
		Update(): calculus_done = 0: Concave_Source()
	EndIf
	ScreenSet work_page, visible_page
	PCopy 2, work_page
End Sub
...
Sub Curve_Display()
	...
	Line(0, y_height + 58)-(1279, y_height + 200), background, bf                 'erasing previous curves.
	Line(0, 0)-(1279, y_height), gray, bf                                         'darker area.
	Line(640 - 116, 458)-(640 + 116, 484), background, bf
	Line(640 - 116, 458)-(640 + 116, 484), black, b
	Locate 30, 68: Print "The Airy Disk Energy Curve"
...
'	----------------------------------------------------------------------------- SIMPLIFIED CALCULUS FOR THE OPTICAL AXIS ONLY
	For x = 0 To x_width / 2                                                      'x = 0  at the center of a sphere.
	   distance = x / lambda
		path_difference = distance - Int(distance)                                 'never exceeds one wavelength.
		phase_1 = 2 * pi * path_difference                                         'on-axis reference period.
	   ...
	   luminance(x, 0) = normalize * Sqr(sine_amplitude * sine_amplitude + cosine_amplitude * cosine_amplitude)  'Pythagoras.
		period(x, 0) = Atn(cosine_amplitude / sine_amplitude)                       'absolute wave period for this pixel.
		If sine_amplitude < 0 Then period(x, 0) = period(x, 0) + pi                 'validating all 4 quadrants.
		period(x, 0)= -period(x, 0) + pi                                            'the axial calculus (y = 0) reverses the period.
		axial_amplitude(x) = .195 * luminance(x, 0)
		axial_period(x) = phase_1 - period(x, 0)                                    'wave period fluctuations.
	...
	If lambda >= 4 Then
		...
		Next
	EndIf
	Line(x_center, y_curve_1)-(x_center, y_curve_1 + 20), red                       'red landmarks for the Airy Disk first zero amplitude points.
	...
	Line(1279 - y_center + small_radius, y_curve_1 + 10)-(1279 - y_center + small_radius, y_curve_1 + 20), red
	Line(0, y_curve_1)-(1279, y_curve_1), black
	Line(x_width, y_height)-(x_width, 719), black
End Sub
...
Sub Display() '***************************************** DISPLAYING THE GRAPHICS
	...
	If distance Then
		Locate 35, 106: Print "Distance to Focal Plane.. ";
...
Sub Initialization()'******************************************** INITIALIZATION
	...
	y_center = y_height / 2
	aperture_angle = 30                                                           'cone angle as referred to the optical axis.
   source_angle = aperture_angle / (180 / pi)
   lambda = 24
	'If aperture_angle <= 90 Then lambda = x_center / 3 / (2 / Sin(source_angle)^2 - 1) Else lambda = 130
	images = 64
	...
	mouse_line_58 = " A - Energy Curve.        ": Locate 58, 39: Print mouse_line_58
	...
	Locate 54, 70: Print "Select Energy Curve mode to modify aperture angle or wavelength faster."
	...
	Locate 62, 121: Print "October 12, 2010. This program may be"
...
Sub Update()'****************************************** UPDATING CURRENT CHOICES
	ScreenSet 2, 2                                                                'hidden matrix page number = 2.
	in_key = ""
	source_angle = aperture_angle / (180 / pi)
	If aperture_angle > 89 Then
	...
	End Select
	Locate 54, 39: Print " Brightness.......";: Print Using " #.# "; brightness
	Locate 58, 39: Print mouse_line_58
   ...
   Locate 35, 2: Print "These graphics were generated by the summation of a large number of Huygens' wavelets."
	Locate 45, 99: Print "glafreniere.com       Gabriel LaFreniere     October 12, 2010."
	Locate 45, 2
	Select Case mode
		Case "gray"
	...
	Color RGB(0,0,255)
	Select Case mode
		Case "curve":  Locate 58, 39: Print mouse_line_58
	...
	PCopy 2, 0
	PCopy 2, 1
	ScreenSet visible_page, visible_page
End Sub
