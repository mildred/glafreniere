...
Dim Shared As Integer bitmap, y_height = 502, x_width = 776, x_center, y_center, aperture_angle
...
Dim Shared As Integer capture, circle_number, cursor, minimum, maximum, previous_angle
Dim Shared As Integer lambda, previous_lambda, half_lambda
...
Dim Shared As Single rotation, amplitude, phase, shade, angle_step, radius, arc_radius
Dim Shared As Single angle, source_angle, pixel_angle, scan_angle, arc_angle, relative_aperture
...
Do
	...
	If Len(in_key) Then
		...
		Select Case in_key
			Case "B": If mode <> "black_and_white" Then mode = "black_and_white": Update()
	...
	Select Case mouse_line
		Case 52: Print deleting: Locate 52, 39
			Print " Aperture Angle..."; aperture_angle; Chr(248); " "
		...
		Case 59: If mode = "black_and_white" Then Else Print mouse_line_59
	...
	If click = 1 Then	'---------------------------------------------------------- MOUSE MANAGEMENT.
		If x_mouse > 0 And x_mouse < x_width And y_mouse < y_height Then x_focal_plane = x_mouse
		If x_mouse > x_width And y_mouse < y_height Then x_focal_plane = x_width
		bitmap = 0: image = 0
		Select Case mouse_line
			Case 52: Cursor_Angle()
			Case 53: Cursor_Lambda()
			Case 54: Cursor_Brightness()
			Case 59: If mode <> "black_and_white" Then mode = "black_and_white": Update()
		...
		End Select
	End If
	If click = 2 And x_mouse > 0 And x_mouse < x_width And y_mouse < y_height Then x_focal_plane = x_center
...
Sub Bitmaps()'*************************************** CREATING A BITMAP SEQUENCE
   ...
   Locate 52, 70: Print file_name
   Locate 54, 70: Print "Warning! A bitmap sequence is being created in the current directory."
	Color black, background
	BSave file_name,0
	If image <= lambda Then x_focal_plane += 1
	If image = 228 Then mode = "color": Update(): x_focal_plane = x_center + lambda
	If image >= 328 And x_focal_plane > x_center Then x_focal_plane -= 1		
	If image = 640 Then mode = "black_and_white": Update()
	capture = capture + 1
	If image > 768 Then End
...
'	----------------------------------------------------------------------------- SIMPLIFIED CALCULUS FOR THE OPTICAL AXIS ONLY
	...
	For y = 0 To y_center
		...
		Energy = .0021 * luminance(0,y) ^ 2
		If y = 0 Then previous_energy = Energy
		Line(4, y_center + y)-(4 + energy, y_center + y), white                    'regular Airy disk energy curve.
		Line(4, y_center - y)-(4 + energy, y_center - y), white
		Line(4 + previous_energy, y_center + y)-(4 + energy, y_center + y), black
		Line(4 + previous_energy, y_center - y)-(4 + energy, y_center - y), black
	...
	Next
	Line(3,0)-(3, y_height), black                                                'base for the focal plane energy curve.
	...
	For x = 1 To x_width / 2                                                      'x = 0 at the center of the sphere.
		...
		If Len(in_key) Then
			If Len(in_key) = 2 Then in_key = Right(in_key, 1) + "+" Else in_key = UCase(in_key)
			Select Case in_key
				'Case "B": If mode <> "black_and_white" Then mode = "black_and_white"
...
Sub Cursor_Angle() '******************************* ADJUSTING THE APERTURE ANGLE
	...
	Do
		...
		ScreenSet work_page, visible_page
		Cls
		If y_mouse > 786 And y_mouse < 866 Then
			aperture_angle = (x_mouse - minimum) / 2
		Else
			aperture_angle = previous_angle
		End If
		If aperture_angle < 5 Then aperture_angle = 5 Else If aperture_angle > 180 Then aperture_angle = 180
		source_angle = aperture_angle / (180 / pi)
		cursor = minimum + 2 * aperture_angle
		Line(minimum + 9, 815)-(maximum, 831), black, b
...
Sub Cursor_Lambda() '********************************** SELECTING THE WAVELENGTH
	...
	Do
		...
		If y_mouse > 802 And y_mouse < 882 Then
			lambda = Int((x_mouse - 200) / 2)
		Else lambda = previous_lambda
		End If
		If lambda > 200 Then lambda = 200 Else If lambda < 1 Then lambda = 1
		GetMouse x_mouse, y_mouse, , click
	Loop While click = 1
	If previous_lambda = lambda Then Else Update(): calculus_done = 0
End Sub


Sub Display() '***************************************** DISPLAYING THE GRAPHICS
	If calculus_done = 0 Then Concave_Source()
	phase = 2 * pi * image / images
	Select Case mode
		Case "color":     Color_Display()
		Case "black_and_white": Black_and_White_Display()
		Case "energy" :   Energy_Display()
	End Select
	Locate 35, 106: Print "Focal Plane Distance...";
	Print Using "##.## lambda (cross section)."; Abs((x_focal_plane - x_center)/ lambda)
...
Sub Energy_Display()'**************************** BLACK AND WHITE ENERGY DISPLAY
   ...
   y_previous = y_curve_1 - .195 * luminance(0, 0)                               'optical axis amplitude graphic.
	...
	For x = 0 To x_center Step lambda                                             'wavelength scale.
   ...
   y_previous = y_curve_1 - .195 * luminance(x_coord, 0)
	...
	Line(1279 - y_center + small_radius, y_curve_1 + 10)-(1279 - y_center + small_radius, y_curve_1 + 20), red
	For y = 0 To y_center Step lambda                                             'wavelength scale.
...
Sub Initialization()'******************************************** INITIALIZATION
	WindowTitle "The Airy Disk Pattern Using the Huygens Principle."
	mode = "energy"
	x_focal_plane = 0
	calculus_done = 0                                                             'set calculus_done = 1 if the calculus is no longer required.
	brightness = 1
	aperture_angle = 90                                                           'cone angle as referred to the optical axis.
	...
	deleting =      "                      "
	Line(295, 777)-(500, 867), black, b
	Line(x_width - 1, 0)-(x_width, 719), black, b                                 'separator.
	Locate 34,106: Print "Wavelength (lambda)...."; lambda; " pixels."
	Locate 50, 40: Print "Click to adjust:"
	Locate 64, 39: Print " P - Pause.";
	Locate 45,100: Print "Gabriel LaFreniere      glafreniere.com      Sept. 28, 2010."
	Line(0, 720)-(1280, 720), black
	Color RGB(0,150,0), background
	Locate 62, 121: Print "September 29, 2010. This program may be"
...
Sub Update()'****************************************** UPDATING CURRENT CHOICES
	...
	in_key = ""
	If x_focal_plane = 0 Then x_focal_plane = x_center
	...
	Else
		small_radius = lambda * (1 + .226 * (Cos(source_angle))) / (2 * Sin(source_angle))'1.226 more accurate than the well known 1.22 Airy number.
	...
	End If
	Locate 33, 106: Print "Airy Disk..............";
	Print Using"##.## lambda (small radius)."; small_radius / lambda
	Locate 33, 29: Print "Airy Ellipsoid...";
	Print Using"###.# lambda (large radius on optical axis)."; large_radius / lambda
	Locate 33, 2: Print "Aperture Angle...."; aperture_angle; Chr(248); " "
	Locate 34, 2: Print "Bilateral Angle..."; 2 * aperture_angle; Chr(248); " "
	If aperture_angle > 89 Then
		relative_aperture = 0
	Else relative_aperture = .5 / Tan(source_angle)
	End If
	...
	EndIf
	Locate 52, 39: Print " Aperture Angle..."; aperture_angle; Chr(248); " "
	Locate 53, 39: Print " Wavelength......."; lambda; "  "
	Locate 54, 39: Print " Brightness.......";: Print Using " #.#"; brightness
	...
	Select Case mode
		...
		Case "color"
			Print "The cyan curve indicates the departure from the axial wavelet normal phase."
		Case "energy"
			Print "Energy distribution on the optical axis.                                   "
			Locate 35, 2: Print "This image was generated by the summation of a large number of Huygens' wavelets."
	End Select
	Color RGB(0,0,255)
	Select Case mode
		Case "black_and_white": Locate 59, 39: Print mouse_line_59
