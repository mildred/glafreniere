...
' JOCELYN MARCOTTE'S 2-D WAVE ALGORITHM (CREATED IN 2006).            "THE PAST IS A GUIDE TO THE FUTURE"
    ...
    For x = x_start To x_stop: For y = y_start To y_stop
      ...
      trend(x,y) = .5 * orthogonal - past(x,y)                        'this simpler trend extrapolation
    Next: Next                                                        'calculus produces wave_speed = .7071

    If iteration Mod Int(lambda / 4 / g_Lorentz / wave_speed)= 0 Then 'deducing energy from quadrature in spite of Doppler.
      For x = 0 To x_width: For y = 0 To y_height
        pulse(x,y) = quadrature(x,y)
      Next: Next      
      For x = 0 To x_width: For y = 0 To y_height
        quadrature(x,y) = present(x+1,y)^2
      Next: Next      
    End If
...
Sub Initialization()
  Windowtitle " The Bradley Aberration of Light - Parallax"
  display = "amplitude"
  iteration = 0
  brightness = 1
  x_center = .5 * x_width
...
' Display.
  ...
  Locate 53, 3: ? "The monitor should support at least 1280x1024 and match the same aspect ratio (5:4)."
  Color dark_gray
  Locate 47, 3: ? "Thanks to the creators of FreeBASIC."
  Locate 48, 3: ? "Gabriel LaFreniere  glafreniere.com";
  Locate 47,100: ? "February 24, 2009."
...
Sub Parabolic_Reflectors()
  For x = -.3 * g_Lorentz * path_width To .3 * g_Lorentz * path_width
    ...
    If frame = 0 Then
      Pset((x + x1_small_parabola) / scale, (y_small_parabola - y) / scale), white 'drawing the reflectors.
...
' In addition, it is slowed down because the frame of reference is also moving forward according to: wave_speed * beta.
  If beta = .5 Then
    ...
    x -= 1555                                                         'adjusting position according to the delay.
    Line(x,0)-(x, 308), white                                         'phase wave position for upper plane mirror.
...
'    Color white
  ...
  If beta = 0 Then
    Locate 39, 1:       ? "     The Bradley      "
    Locate 40, 1:       ? "  Aberration of Light "
  ...
  Else
    ...
    Locate 42,67: ? "Parabolae including distance"
    Locate 43,43: ? "between them contract. Parallax remains symmetrical in spite of the Doppler"
    Locate 44,43: ? "   effect. The contracted angle is unnoticeable for the moving observer."
    Locate 45,43: ? "A time shift occurs between focal planes according to the phase wave speed."
  End If
