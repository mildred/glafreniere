...
Dim Shared As Integer x_coord, y_coord, x_squared, x_circular_emitter, x_straight_emitter
...
Dim Shared As Single orthogonal, diagonal, influence, potential, k_Dewavrin, kinetic, Lagrangian, quadrature, factor
...
'     trend(x,y) = .5 * orthogonal - past(x,y)                        'simpler but slower trend extrapolation.
    Next: Next
...
' Display.
  ...
  Locate 47,89: ? "November 28, 2008. This program may be"
...
Sub Wave_Generator_Circular()
  ...
  For x = -lambda / 4 To lambda / 4
    For y = -lambda / 4 To lambda / 4
      trend(x_circular_emitter+x,y_center+y)=trend(x_circular_emitter+x,y_center+y)+potential*circular_amplitude(x,y)
    Next
  Next
End Sub
...
Sub Wave_Generator_Straight()
  ...
  For y = 0 To y_height
    ...
    For x = -lambda / 4 To lambda / 4
      trend(x+x_straight_emitter,y) = trend(x+x_straight_emitter,y) + potential * straight_amplitude(x)'straight wave.
    Next
  Next
End Sub
