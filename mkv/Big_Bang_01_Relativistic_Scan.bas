...
Dim Shared As Single alpha, beta, beta_1, beta_2, beta_3, beta_x, beta_y
Dim Shared As Single g_Lorentz, g_alpha, g_Lorentz_1, g_Lorentz_2, g_Lorentz_3, g_Lorentz_x, g_Lorentz_y
Dim Shared As Single x_A, x_coord, y_coord, x_prime, y_prime, t_time, t_prime, t_Lorentz, t_start
Dim Shared As Single k_Dewavrin, move_frame, frame_speed, c_speed, beta_prime
...
wave_display = 0: skipped_frames = 20: scanner = 1: bitmap = 0         'set bitmap = 1 for BMP image sequence.
Initialization()
...
'        trend(x,y) = .4 * (orthogonal + present(x,y)) - past(x,y)    'slower wave speed: .4 * (4 + 1) - 1 = 1
    ...
    twins_distance = beta_1 * t_Lorentz
    If scanner Then move_frame += c_speed * alpha
    If move_frame > 1 Then move_frame -= 1: Frame_of_Reference()
    x_A = x_A_Int - move_frame
    x_B = x_A + beta_1 * t_Lorentz                                    'B is moving rightward (beta = 0.8 c).
    x_C = x_A + beta_2 * t_Lorentz                                    'C is moving rightward (beta = 0.8 c).
    ...
    End If
    If iteration > 190 Then wave_display = 1: pulse_B = 1: Emitter_B()
...
Sub Display()
  ...
  y_D = y_center - beta_1 * t_prime
  y_E = y_center + beta_1 * t_prime
' beta = .5 * (.5 * beta_2 + beta_1)                                  'the ellipse contracts according to .45 c: (.4 + .5) / 2.
' g_Lorentz = Sqr(1 - beta ^ 2)                                       'mean contraction for expanding (A to C) and moving ellipsoidal mirror. 
' x_coord = .5 * (x_A + x_C)                                          'center of the expanding ellipse.
' Circle(x_coord,y_center),.5*(x_C-x_A)/g_Lorentz,white,,, 1/g_Lorentz'expanding ellipsoid mirror.
  For radian = 0 To 2 * pi Step pi / sphere_radius                    'expanding ellipsoid mirror - see "Elliptic_Reflector()".
      x_coord = beta_1 * sphere_radius * Cos(radian)                  'axial coordinate before transformation (proportional to speed).
      y_coord = beta_1 * sphere_radius * Sin(radian)                  'transverse coordinate before transformation.
      ...
      beta = (beta + beta_1) / (1 + beta * beta_1)                  'accelerating the current spot according to beta_1.
      g_Lorentz = Sqr(1 - beta ^ 2)                               'Lorentz contraction factor for the current spot.
  ...
  y_prime = g_Lorentz_1 * y + beta_1 * t_prime
  ...
  If scanner = 0 Then                                                 'the scanner misses thin vertical lines.
    Circle(x_A, y_center), sphere_radius, gray,,, beta_1              'horizontal .5 c position (ellipses).
    Circle(x_A, y_center), sphere_radius, gray,,, 1 / beta_1          '  vertical .5 c position.
    Circle(x_A, y_center), sphere_radius, gray,,, beta_2              'horizontal .8 c position.
    Circle(x_A, y_center), sphere_radius, gray,,, 1 / beta_2          '  vertical .8 c position.
    Circle(x_A, y_center), sphere_radius, gray,,, beta_3              'horizontal .92857 c position.
    Circle(x_A, y_center), sphere_radius, gray,,, 1 / beta_3          '  vertical .92857 c position.
    Line(x_A, y_center - sphere_radius)-(x_A, y_center + sphere_radius),gray 'vertical y axis.
  End If

  Line(x_A, y_center)-(x_C, y_center),gray                            'horizontal x axis (A to C).
...
'       beta_y = 0
'       Print_Galaxy()                                                'print left side only below.
        beta_y = beta_1
        Print_Galaxy()
        beta_y = beta_2
        Print_Galaxy()
  ...
  Circle(x_A, y_center), radius, 12,,,1                               'central unmoving white galaxy A.
  ...
  Color white, dark
  Locate 12, 04: ? "x'= g * x + b * t"
  Locate 13, 04: ? "t'= g * t - b * x"
...
Sub Elliptic_Reflector()                                              'see display() above for calculus details.
  For radian = 0 To 2 * pi Step pi / sphere_radius
    x_coord = beta_1 * (sphere_radius+0) * Cos(radian)
    y_coord = beta_1 * (sphere_radius+0) * Sin(radian)
  ...
  For x = -.25 * lambda To .25 * lambda
    ...
    For y = -.25 * lambda To .25 * lambda
      ...
      If distance < .25 * lambda Then
        ...
        trend(x_A + x, y_center + y) = trend(x_A + x, y_center + y) + curve
...
Sub Emitter_B()                                                       'axial emitter B: beta = 0.5.
  ...
  If t_start = 0 Then t_start = g_Lorentz_1 * t_time - .75            'start when amplitude is maximum where x = 0.
  t_prime = g_Lorentz_1 * t_time                                      'Lorentz's t' time for x = 0.
...
' Locate 20,20: Print t_prime - t_start
  ...
  For x = -.25 * g_Lorentz_1 * lambda To .25 * g_Lorentz_1 * lambda
    x_squared = (x / g_Lorentz_1)^2                                   'absolute axial distance squared in pixel units.
    x_coord = x / g_Lorentz_1 / lambda                                'Lorentz's x coordinate in wavelength units.

'*********************************************************************
' This is my reversed version of the Lorentz transformations.
    x_prime = g_Lorentz_1 * x_coord + beta_1 * t_time                 'Lorentz's x' coordinate in lambda units.
    t_prime = g_Lorentz_1 * t_time  - beta_1 * x_coord                'Lorentz's t' time in wave period units.
'*********************************************************************
    
    x_prime = x_A + x_prime * lambda                                  'Lorentz's x' coordinate in pixels for twin B, relative to twin A.
...
' APPLYING THE BRAND NEW TRIDIMENSIONAL LORENTZ TRANSFORMATIONS.
  x_prime = g_Lorentz_x * x + beta_x * t_Lorentz
  t_prime = g_Lorentz_x * t_Lorentz - beta_x * x
  y_prime = g_Lorentz_y * y + beta_y * t_prime                        'new y' equation (should also work as z' in 3-D).
'*********************************************************************
