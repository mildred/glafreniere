...
? " Created March 5, 2008 by Gabriel LaFreniere.":?:?
...
Declare Sub Parabolic_Reflectors()
...
Const gold = Rgb(180,150,100), buff = Rgb(255,255,200), blue_sky = Rgb(210,230,255), orange = Rgb(255,100,0)
...
x_screen = 1280 * scale
y_screen = 1024 * scale
Dim Shared As Integer x_start, y_start, x_stop, y_stop, x_center, y_center, x_pixel, y_pixel, x_scanner, y_scanner
Dim Shared As Integer x_coord, y_coord, x_squared, x_straight_emitter, x1_small_parabola, x2_small_parabola, x_mouse, y_mouse
Dim Shared As Integer y_parabola, y_small_parabola, y_beam_splitter, x_vertical_mirror, h_height, x_emitter, y_emitter
...
Dim Shared As Single wave_speed, ratio, brightness, move_frame, t_time, t_prime, g_Lorentz, Doppler, k_Dewavrin
Dim Shared As Single orthogonal, diagonal, influence, potential, kinetic, Lagrangian, factor, transparency
Dim Shared As Single beta, mirror_angle, amplitude, phase, distance, radian, previous, previous_phase, single_variable
...
lambda = 20 * scale
beta = .517593  'according to wave_speed * (1 / beta - beta) = 1 in order to obtain a 1 pixel per loop phase wave speed. 
...
' JOCELYN MARCOTTE'S 2-D WAVE ALGORITHM (CREATED IN 2006).            "THE PAST IS A GUIDE TO THE FUTURE"
    ...
    Next: Next                                                        'produces a slower wave speed: c = .7071 pixel.
    ...
    Parabolic_Reflectors()
    Flat_Mirrors()
    Damping_Management()                                              'processing damping zone.
    Circular_Generator_Doppler()                                      'circular impulses.
    iteration += 1
    Time_Scanner()
    If iteration > 1000 And iteration < 1500 then x_scanner = 0 Else x_scanner += 1
...
Sub Circular_Generator_Doppler()
  ...
  amplitude = 14000 / lambda ^ 2
  
  If stop_pulsating = 0 Then
    If iteration > (pulses - 2) * (lambda / g_Lorentz / wave_speed) Then'the wave speed is .5 pixel per loop.
...
'  x = iteration * (wave_speed / beta - wave_speed * beta)             'phase wave (Lorentz's t' = 0) position.
'  x -= 1000                                                           'adjusting position according to the delay.
...
Sub Initialization()
  Windowtitle " The Bradley Aberration of Light - Parallax & Scan"
  display = "amplitude"
  iteration = 0
  brightness = 1
  x_center = .5 * x_width
  ...
  damping_zone = 50 * Sqr(lambda * (1 + beta) / g_Lorentz)            'according to backward Lorentz Doppler.
  damping_zone = 30 * Sqr(lambda * (1 + beta) / g_Lorentz)            'according to backward Lorentz Doppler.
  If damping_zone > 400 Then damping_zone = 400
  off_axis = damping_zone
  path_width = g_Lorentz * x_center + damping_zone - 2
  If x_width < y_height Then x_width = y_height
  parabola_radius = 2 * .92 * y_height
  y_parabola = y_height
  y_small_parabola = .92 * y_height
  x1_small_parabola = x_center - .2 * g_Lorentz * x_width
  x2_small_parabola = x_center + .2 * g_Lorentz * x_width
  small_parabola_radius = .8 * y_small_parabola
  x_emitter = x_center
  x_scanner = 0: y_scanner = 600
...
' Display.
  ...
  Locate 47,100: ? "March 5, 2009."
  Locate 48,100: ? "This program may be freely copied, distributed, or modified."
  Color green_text
...
Sub Parabolic_Reflectors()
  
  For x = -.3 * g_Lorentz * path_width To .3 * g_Lorentz * path_width
    single_variable = (x / g_Lorentz) ^ 2 / 2 / small_parabola_radius 'classic parabola sagitta including contraction.
    y = Int(single_variable)                                          'depixellation according to accurate parabola position.
    single_variable -= y
  ...
  If frame = 0 Then                                                   'drawing the reflectors.
    For x = -.3 * g_Lorentz * path_width To .3 * g_Lorentz * path_width Step scale
      y = (x / g_Lorentz) ^ 2 / 2 / small_parabola_radius             'squashed parabolae.
      Pset((x + x1_small_parabola) / scale, (y_small_parabola - y) / scale), white
      Pset((x + x2_small_parabola) / scale, (y_small_parabola - y) / scale), white
    Next
  End If
End Sub
...
Sub Wave_Display()
  ...
  If display = "amplitude" Then                                       'show wave amplitude in red and green colors.
    For x = 0 To x_scanner Step scale: For y = 0 To y_height Step scale
      Pset(x / scale, y / scale), scanned_color(x,y)
    Next: Next

    For x = x_scanner To x_width Step scale: For y = 0 To y_height Step scale
      luminance_1 = brightness * Abs(trend(x-1,y) + trend(x,y) + trend(x+1,y))
      b = luminance_1 / 2
      If b > 255 Then b = 255
      If luminance_1 > 255 Then
        luminance_2 = luminance_1 - 255
  ...
  Line(x1_small_parabola / scale, (y_small_parabola - 300) / scale)-(x1_small_parabola / scale, y_small_parabola / scale), gray,,&HFF
  Line(x2_small_parabola / scale, (y_small_parabola - 300) / scale)-(x2_small_parabola / scale, y_small_parabola / scale), gray,,&HFF
  Line(x_emitter / scale, y_emitter / scale)-(x1_small_parabola / scale, y_small_parabola / scale), gray
  Line(x_emitter / scale, y_emitter / scale)-(x2_small_parabola / scale, y_small_parabola / scale), gray
  Line((2*x1_small_parabola-x_emitter) / scale, y_emitter / scale)-(x1_small_parabola / scale, y_small_parabola / scale), gray
  Line((2*x2_small_parabola-x_emitter) / scale, y_emitter / scale)-(x2_small_parabola / scale, y_small_parabola / scale), gray
  
'  Line(x_center / scale-10, y_emitter / scale-1)-(x_center / scale+10, y_emitter / scale+1), black, b
'  Line(x_center / scale-10, y_emitter / scale)-(x_center / scale+10, y_emitter / scale), white

' The phase wave speed is given by 1 / beta. A correction is added because the true wave speed is .5 pixel per loop.
' In addition, it is slowed down because the frame of reference is also moving forward according to: wave_speed * beta.

  x = iteration * (wave_speed / beta - wave_speed * beta) / scale     'phase wave (Lorentz's t' = 0) position.
  x -= 1500                                                           'adjusting position according to the delay.
  If x > x_width Then x = x_width
  If x > 0 Then x_scanner = x
'  Line(x-20,288)-(x+20, 288), white
  Line(x_scanner,0)-(x_scanner, y_scanner), orange                'phase wave position for the Time Scanner.

  Color white, black
  Locate 39, 155: Print Using "#####"; Int(iteration / (skipped_frames + 1));
  Locate 37, 1:       ? "    The Bradley      "
  Locate 38, 1:       ? " Aberration of Light "
  Locate 40, 1:       ? " PARALLAX and motion "
  Locate 42, 1: ? Using " Beta......... #.####  v/c ";beta
  Locate 43, 1:       ? " Theta........";
                ? Using "##.##"; Asin(beta) * 180 / pi;: ? Chr(248); "   arc sin(beta) "
  Locate 44,1
  If contraction = 0 Or beta = 0 Then
'    Color red
    ? " No Contraction (regular parabolae)."
'    Color white
  Else           ? Using " Contraction.. #.####  sqr(1-beta^2) "; g_Lorentz
  End If  
  Locate 45,1:   ? " Lambda......."; lambda / scale; " pixels (wavelength) "

  Locate 42, 67: ? "Thanks to the phase wave, the"
  Locate 43,43:  ? "Time Scanner cancels Lorentz's time shift in such a way that the Doppler" 
  Locate 44,43:  ? "effect is also canceled. Lorentz's contraction is still present but it may" 
  Locate 45,43:  ? "easily be corrected by stretching the image according to Lorentz's factor." 
  Locate 41,142: ? " Special thanks to "
  Locate 42,138: ? " Philippe Delmotte and "
  Locate 43,129: ? " Jocelyn Marcotte, the creators "
  Locate 44,124: ? " of this amazing virtual wave medium."
  Locate 45,124: ? " Gabriel LaFreniere  glafreniere.com ";
  Locate 50, 99: ? " Iteration"; iteration; " ";
End Sub
