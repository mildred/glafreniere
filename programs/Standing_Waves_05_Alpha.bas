...
Dim Shared As Single radian, wave_speed, brightness, lambda_1, lambda_2
Dim Shared As Single alpha, g_alpha, beta, g_beta, ratio, lambda_prime_1, lambda_prime_2
Dim Shared As Single orthogonal, diagonal, amplitude, phase, distance, Doppler
Dim Shared As Single image(0 To 260, 213 to 313)
...
' ********************************************************************
    ...
    iteration += 1
    If iteration > 2393 Then Sleep: End
  ...
  If bitmap > 0 And iteration > 700 Then
    Select Case bitmap
...
Sub Initialization()
  Windowtitle " The Hertz Test and the Alpha Mean Speed"
  x_center = .5 * x_width
  ...
  x_emitter_1 = 47
  g_beta = Sqr(1 - beta^2)
  alpha = (1 - g_beta) / beta                                         'alpha = .2679492
  g_alpha = Sqr(1 - alpha^2)
  Doppler = lambda * (1 + beta) / g_beta
  lambda_1 = lambda  * (1 + alpha) / (1 - alpha)
  lambda_2 = Doppler * (1 - alpha) / (1 + alpha)
  lambda_prime_1 = g_alpha * Sqr(lambda * lambda_1)
  lambda_prime_2 = g_alpha * Sqr(Doppler * lambda_2)
  ...
  Bload "Standing_Waves_05_Alpha.bmp"
  Locate 50, 2: Print "screen speed alpha ";
  Print Using " #.######"; alpha;
  Print Using "  g = #.######";  g_alpha;
  Locate 51, 2: Print "emitter speed beta ";
  Print Using " #.######"; beta;
  Print Using "  g = #.######";  g_beta;
  Locate 52, 2: Print "    lambda forward "; lambda
  Locate 53, 2: Print "   lambda backward ";
  Print Using " ###.####"; Doppler
  Locate 54, 2: Print "          lambda 1 "; lambda_1
  Locate 55, 2: Print "          lambda 2 "; lambda_2
  Locate 56, 2: Print "    lambda prime 1 "; lambda_prime_1;: ? lambda * (1 + alpha)
  Locate 57, 2: Print "    lambda prime 2 "; lambda_prime_2;: ? Doppler * (1 - alpha)
  ...
  Locate 63,121: ? "August 25, 2010. This program may be"
  ...
  Line(47, 104)-(47 + 2 * lambda, 105), black, b                      'wavelength scale.
  For x = 47 To 2 * lambda + 47 Step lambda
    Line(x-1, 105)-(x+1, y_start - 3), black, bf
  Next
  For x = 47 + lambda / 2 To 2 * lambda + 47 Step lambda
    Line(x-1, 105)-(x+1, y_start - 6), black, bf
  Next
  Line(400, 104)-(400 + Doppler, 105), black, b
  For x = 400 To 400 + Doppler Step Doppler
    Line(x-1, 105)-(x+1, y_start - 3), black, bf
  Next
  For x = 400 + Doppler / 2 To 400 + 1 * Doppler Step Doppler
    Line(x-1, 105)-(x+1, y_start - 6), black, bf
  Next
  Line(0, y_curve + 52)-(260, y_curve + 52), black
  For x = 0 to 260
    For y = 213 To 313
      image(x, y) = Point(x, y)
    Next
  Next
  Line(94, y_curve - 154)-(260, y_curve - 54), blue_sky, bf
End Sub
...
Sub Linear_Emitters()
  x_emitter_2 = Int(iteration * beta  + 47)
  If iteration < 1000 Then Exit Sub
  phase = (iteration - 1000) * 2 * pi / lambda
  amplitude = 1360 * Cos(phase) / lambda ^ 2
  For x = -lambda / 8 To lambda / 8
    trend(x + x_emitter_1) = trend(x + x_emitter_1) + amplitude
  Next
  phase = g_beta * (iteration - 1000) * 2 * pi / lambda
  amplitude = 3400 * Cos(phase) / Doppler ^ 2
  For x = -lambda / 8 To lambda / 8
...
'*********************************************************************
Sub Moving_Screen()
  x_screen = Int(iteration * alpha + 47)
  ratio = (iteration * alpha + 47) - x_screen                         'the 1-D moving reflector needs buffering.
  trend(x_screen) = 0
...
Sub Wave_Display()
  ...
  Line(x_screen + 5, y_curve - 52)-(x_screen + 3 * lambda_prime_2 + 5, y_curve + 52), dark_gray, bf
  For x = x_screen + 5 To x_screen + 3 * lambda_prime_2 + 5
  ...
  For x = 95 to 183                                                   'printing B (moving screen alpha).
    For y = 213 To 313
      If image(x, y) <> blue_sky Then Pset(x + x_screen - 139, y), image(x, y)
    Next
  Next
  For x = 186 to 260                                                  'printing C (moving emitter beta).
    For y = 213 To 313
      If image(x, y) <> blue_sky Then Pset(x + x_emitter_2 - 223, y), image(x, y)
    Next
  Next
